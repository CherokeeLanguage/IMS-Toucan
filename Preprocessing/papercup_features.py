...
def generate_feature_lookup():
    return {
            ...
            '\u02e5': {
                    'symbol_type': 'tone_letter',
                    'value'      : 'tone_extra_high'
            },  # ◌˥
            '\u02e6': {
                    'symbol_type': 'tone_letter',
                    'value'      : 'tone_high'
...
def generate_feature_table():
    ...
    for ipa in ipa_to_phonemefeats:
        if len(ipa) == 1:
            phone_to_vector[ipa] = [0] * sum(
                    [len(values) for values in [feat_to_val_set[feat] for feat in feat_to_val_set]])
            for feat in ipa_to_phonemefeats[ipa]:
